...
const CommunityDropdown = ({ onSelectCommunity }) => {
            ...
            navigator.geolocation.getCurrentPosition(
                async (position) => {
                    ...
                    try {
                        ...
                        console.error('Error fetching place details:', error);
                        setMessage(t('communityDropdown.errorCouldNotFindDetails'));
                    }
                    setIsRefreshing(false);
                },
                (error) => {
                    console.error('Error getting location:', error);
                    setMessage(t('communityDropdown.errorUnableToRetrieve'));
                    setIsRefreshing(false);
                }
            );
        } else {
            setMessage(t('communityDropdown.errorGeolocationNotSupported'));
            setIsRefreshing(false);
        }
    };
    ...
    return (
        <div className="p-6 bg-gray-100 rounded-lg">
            <h2 className="text-2xl font-bold mb-4">{t('communityDropdown.header')}</h2>
            <select
            ...
            >
                <option value="all">{t('communityDropdown.allCommunities')}</option>
            ...
            >
                {t('communityDropdown.addCommunity')}
            </button>
            {showForm && (
                <form onSubmit={handleFormSubmit} className="mt-6 p-4 bg-white rounded-lg shadow-md">
                    ...
                    {useCurrentLocation && longitude && latitude ? (
                        <>
                            <div>
                                ...
                                <Select
                                    ...
                                    placeholder={t('communityDropdown.selectPlace')}
                            ...
                            {placeDetails && placeDetails.length > 0 && (
                                <div>
                                    <label className="block font-bold mb-1">{t('communityDropdown.postalCode')}</label>
                                    <Select
                                        ...
                                        placeholder={t('communityDropdown.selectPostalCode')}
                                        className="w-full border p-2 rounded-lg"
                                        isSearchable
                                    />
                                </div>
                            ...
                            >
                                <span>{t('communityDropdown.refreshLocation')}</span>
                                {isRefreshing ? (
                                    <div
                        ...
                        <>
                            <div>
                                <label className="block font-bold mb-1">{t('communityDropdown.selectPlace')}</label>
                                <Select
                                    options={placeOptions}
                                    value={placeOptions.find(option => option.value === placeId)}
                                    onChange={(selectedOption) => {
                    ...
                    <button type="submit" className="bg-green-700 text-white py-2 px-4 rounded-lg mt-10">
                        {t('communityDropdown.saveCommunity')}
                    </button>
                </form>
            )}
        </div>
